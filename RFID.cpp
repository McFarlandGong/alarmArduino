...
/*
 *  Read_MFRC522 Nombre de la funci: Read_MFRC522
 *  Descripci: Desde el MFRC522 leer un byte de un registro de datos
 *  Los paretros de entrada: addr - la direcci de registro
...
void RFID::calculateCRC(unsigned char *pIndata, unsigned char len, unsigned char *pOutData)
{
	...
	// Esperar a la finalizaci de cculo del CRC
    i = 0xFF;
    do 
    {
...
unsigned char RFID::MFRC522ToCard(unsigned char command, unsigned char *sendData, unsigned char sendLen, unsigned char *backData, unsigned int *backLen)
{
    ...
    switch (command)
    {
        case PCD_AUTHENT:		// Tarjetas de certificaci cerca
		{
			irqEn = 0x12;
			waitIRq = 0x10;
			break;
		}
		case PCD_TRANSCEIVE:	//La transmisi de datos FIFO
		{
			irqEn = 0x77;
			waitIRq = 0x30;
			break;
    ...
    writeMFRC522(CommIEnReg, irqEn|0x80);	//De solicitud de interrupci
    clearBitMask(CommIrqReg, 0x80);			// Borrar todos los bits de petici de interrupci
    setBitMask(FIFOLevelReg, 0x80);			//FlushBuffer=1, FIFO de inicializaci
...
/*
 *  Nombre de la funci: MFRC522_Request
 *  Descripci: Buscar las cartas, leer el nero de tipo de tarjeta
 *  Los paretros de entrada: reqMode - encontrar el modo de tarjeta,
 ...
 *  Anti-detecci de colisiones, la lectura del nero de serie de la tarjeta de tarjeta
 *  @param serNum - devuelve el nero de tarjeta 4 bytes de serie, los primeros 5 bytes de bytes de paridad
...
unsigned char RFID::anticoll(unsigned char *serNum)
{
    ...
    if (status == MI_OK)
	{
		//?????? Compruebe el n鷐ero de serie de la tarjeta
		for (i=0; i<4; i++)
		{   
...
/* 
 * MFRC522Auth -> auth
 * Verificar la contrase de la tarjeta
 * Los paretros de entrada: AuthMode - Modo de autenticaci de contrase
                 0x60 = A 0x60 = validaci KeyA
                 0x61 = B 0x61 = validaci KeyB
             BlockAddr--  bloque de direcciones
             Sectorkey-- sector contrase
             serNum--,4? Tarjeta de nero de serie, 4 bytes
 ...
 * Los paretros de entrada: blockAddr - direcci del bloque; recvData - leer un bloque de datos
 * MI_OK Valor de retorno: el retorno exitoso MI_OK
 */
 ...
 * blockAddr - direcci del bloque; WriteData - para escribir 16 bytes del bloque de datos
 * Valor de retorno: el retorno exitoso MI_OK
 */
unsigned char RFID::write(unsigned char blockAddr, unsigned char *writeData)
{
    ...
    if (status == MI_OK)
    {
        for (i=0; i<16; i++)		//FIFO 16Byte Datos 
        {    
        	buff[i] = *(writeData+i);   
        }
 ...
 * Los paretros de entrada: Ninguno
